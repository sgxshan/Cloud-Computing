...
@webapp.route('/upload_<UserId>',methods=['POST'])
#upload file in s3
def upload(UserId):
...
@webapp.route('/machine_learning_<UserId>', methods=['GET', 'POST'])
def machine_learning(UserId):
    return redirect("http://34.201.42.249:5000/" + UserId)
...
#     print file_path
#     return render_template("Image/original.html", file = file_path[1:], UserId= UserId)

@webapp.route('/classification_<UserId>_<file>')
def classification(UserId,file):
    ...
    return render_template("Image/predict.html", probability = probability, name=name, file= key, msg=msg)
