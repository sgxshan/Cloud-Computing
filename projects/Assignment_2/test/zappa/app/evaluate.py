...
@webapp.route('/filter<file>_<input>',methods=['GET','POST'])
def filter(file,input):
    folder = 'model'
    all_file = file_list(folder)
    s3_client = boto3.client('s3')
    bucket_name = s3_config
    for FileName in all_file:
        if(file==(os.path.splitext(FileName)[0])):
            file_name = os.path.join('app/static', FileName)
            s3_client.download_file(bucket_name, folder + '/' + file +'.ckpt', file_name)
    print (file)
    print (input)
    parser = build_parser()
    opts = parser.parse_args()
    #check_opts(opts)
    checkpoint_dir = 'app/static/./'+file+'.ckpt'
    print (checkpoint_dir)
    in_path = os.path.join('app/static', input)
    print (in_path)
    in_path = str(in_path)
    print (in_path)
    time = datetime.datetime.now()
    #in_path = input
    #in_path = 'app/static/china.jpg'
    #file_name = os.path.join('app/static', FileName)
    #in_path = file+'.jpg'
    out_path = 'app/static/'+str(time)[20:]+input
    print(out_path)
    if not os.path.isdir(in_path):
        ...
        ffwd_to_img(in_path, out_path, checkpoint_dir,device=opts.device)
    else:
    ...
    return render_template("Image/view.html", file = out_path[4:])
